...
class RBF(object):
    ...
    def train(self, traj, epoch=5000):
        ...
        ag_mean = torch.autograd.Variable(torch.rand(self.Tasks, self.K, self.dim), requires_grad=False)
        ag_logvar2 = torch.autograd.Variable(torch.ones(self.Tasks, self.K, self.dim) * -1, requires_grad=False)
        ag_weight = torch.autograd.Variable(torch.ones(self.Tasks), requires_grad=False) 
        ...
        def PRML_EM():
            ...
            for i in range(self.Tasks):
                ...
                Q = torch.div(pw_normal(ag_w, ag_mean[i], torch.exp(ag_logvar2[i]))*qz[i], 
                    pw_normal_mixture(ag_w, ag_mean, torch.exp(ag_logvar2), ag_weight)) # N
                N_k = Q.sum(0)
                ag_mean[i] = (Q.unsqueeze(1).unsqueeze(1)*ag_w).sum(0)/N_k
                ag_weight[i] = N_k/ag_w.shape[0]
                ag_logvar2[i] = (((ag_w - ag_mean[i])**2)*Q.unsqueeze(1).unsqueeze(1)).sum(0)/N_k
        ...
        for e in range(epoch):
            ...
            if e % 100 == 0:
                ...
                for i in range(self.Tasks):
                    for _ in range(100):
                        taug = self.generate(task=i, fix=False)
