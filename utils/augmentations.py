...
class CDMPAugmentation(object):
    def __init__(self, size=224, current='RGB'):
        self.size = size
        self.augment = Compose([
            ConvertFromInts(),
            PhotometricDistort(current=current),
            RandomGrayscale(current=current),
            Resize(self.size),
        ])

    def __call__(self, img):
        return self.augment(img, None, None)[0]
