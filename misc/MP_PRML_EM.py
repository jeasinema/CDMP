...
class RBF(object):
    ...
    def train(self, traj, epoch=5000):
        ...
        ag_mean = torch.autograd.Variable(torch.rand(
            self.Tasks, self.K, self.dim), requires_grad=False)
        ag_logvar2 = torch.autograd.Variable(torch.ones(
            self.Tasks, self.K, self.dim) * -1, requires_grad=False)
        ag_weight = torch.autograd.Variable(
            torch.ones(self.Tasks), requires_grad=False)
        ...
        def PRML_EM():
            ...
            for i in range(self.Tasks):
                ...
                ag_logvar2[i] = (((ag_w - ag_mean[i])**2) *
                                 Q.unsqueeze(1).unsqueeze(1)).sum(0) / N_k

            pw = torch.log(pw_normal_mixture(
                ag_w, ag_mean, torch.exp(ag_logvar2), ag_weight)).sum(0)
            return pw
